...
function RankBidTable({ data, loading, isApiConfigured, onScraperDataReceived }: RankBidTableProps) {
  ...
  return (
    <div className="glass-card p-6">
      ...
      {/* 변형 키워드 순위별 입찰가 */}
      {data?.variant_data && (
        (data.variant_data.pc_rank_bids?.length > 0 || data.variant_data.mobile_rank_bids?.length > 0) && (
          <div className="mt-6 pt-6 border-t border-dark-border">
            <h3 className="text-base font-semibold mb-4">
              변형 키워드 순위별 입찰가
              <span className="ml-2 text-naver-green font-normal">"{data.variant_data.keyword}"</span>
              {data.variant_data.rank_bids_estimated && (
                <span className="ml-2 text-xs font-normal text-yellow-400 bg-yellow-400/10 px-2 py-0.5 rounded-full border border-yellow-400/30">
                  추정값
                </span>
              )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-dark-border">
                    <th className="px-4 py-2 text-left text-dark-muted">순위</th>
                    <th className="px-4 py-2 text-right text-dark-muted">PC 입찰가</th>
                    <th className="px-4 py-2 text-right text-dark-muted">모바일 입찰가</th>
                  </tr>
                </thead>
                <tbody>
                  {ranks.map((rank) => {
                    const pcBid = getBidForRank(data.variant_data?.pc_rank_bids, rank);
                    const mobileBid = getBidForRank(data.variant_data?.mobile_rank_bids, rank);
                    return (
                      <tr key={rank} className="border-b border-dark-border/50 hover:bg-dark-hover transition">
                        <td className="px-4 py-3 font-medium">
                        ...
                        <td className="px-4 py-3 text-right text-white font-medium">
                          {data.variant_data?.rank_bids_estimated && pcBid != null ? (
                            <span>
                              {pcBid.toLocaleString()}원
                              <span className="ml-1 text-xs text-yellow-400">(추정)</span>
                            </span>
                        ...
                        <td className="px-4 py-3 text-right text-white font-medium">
                          {data.variant_data?.rank_bids_estimated && mobileBid != null ? (
                            <span>
                              {mobileBid.toLocaleString()}원
                              <span className="ml-1 text-xs text-yellow-400">(추정)</span>
                            </span>
